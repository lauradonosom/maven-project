        ...
        stage ('Deployments'){
            parallel{
                stage('Deploy yo staging'){
                    steps{
                        sh "scp -r -i /var/lib/jenkins/.ssh/lara.pem **/target/*.war ec2-user@${params.tomcat_dev}:/var/lib/tomcat7/webapps"
                    }
                }
                stage('Deploy yo production'){
                    steps{
                        sh "scp -r -i /var/lib/jenkins/.ssh/lara.pem **/target/*.war ec2-user@${params.tomcat_prod}:/var/lib/tomcat7/webapps"
                    }
                }
            }
        }
